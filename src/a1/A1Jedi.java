...
public class A1Jedi {
	...
	public static void main(String[] args) {
		...
		for (int j = 0; j < customerCount; j++) {
			boolean[] trueOrFalse = new boolean[foodNameArray.length];
			for (int h = 0; h < trueOrFalse.length; h++) {
				trueOrFalse[h] = false;
			}
			 ...
			 * and if the nameOfItem is equal to the array in the foodName array, add it the
			 * quantityOfItem to a new array and store it in the same place as the
			 * foodNameArray.
			 */
		...
		for (int i = 0; i < foodNameArray.length; i++) {
			if (foodCountArray[i] == 0) {
				...
				System.out.println(
						numberOfCustomersBought[i] + " customers bought " + foodCountArray[i] + " " + foodNameArray[i]);
			}
			scan.close();
